...
class CompositeIndex(GraphIndexer):
    ...
    def __init__(self, **kwargs):
        ...
        print(f"element to index is {self.element_to_index}")
        print(kwargs)

    def __str__(self):
        return "CompositeIndex"

    def create_get_indices_by_label_request(self):
        if str(self.element_to_index) == "VertexLabel":
            if self.ELEMENT is not None:
                ...
                raise AttributeError("Element needs to be set using set_element() "
                                     f"when creating indices by label. Found {self.ELEMENT}")
    ...
    def create_enable_index_by_name_request(self):
        if str(self.element_to_index) == "VertexLabel":
            index = management_pb2.CompositeVertexIndex(name=self.index_name, label=self.ELEMENT.name)
            self.REQUEST = management_pb2.EnableVertexCompositeIndexByNameRequest(context=self.CONTEXT, index=index)
        else:
            index = management_pb2.CompositeEdgeIndex(name=self.index_name, label=self.ELEMENT.name)
            self.REQUEST = management_pb2.EnableEdgeCompositeIndexByNameRequest(context=self.CONTEXT, index=index)
            
        return self

    def create_put_index_by_label_request(self):
        if str(self.element_to_index) == "VertexLabel":
            if self.ELEMENT is not None:
    ...
    def create_put_index_across_labels_request(self):
        if str(self.element_to_index) == "VertexLabel":
            vp = self.__generate_vertex_properties__()
            print(self.index_name, " === " , vp, " === " , self.unique_index)
            index = management_pb2.CompositeVertexIndex(name=self.index_name, properties=vp, unique=self.unique_index)
            self.REQUEST = management_pb2.EnsureCompositeIndexForVerticesRequest(context=self.CONTEXT, index=index)

        elif str(self.element_to_index) == "EdgeLabel":
            ep = self.__generate_edge_properties__()
            index = management_pb2.CompositeEdgeIndex(name=self.index_name, properties=ep)
            self.REQUEST = management_pb2.EnsureCompositeIndexForEdgesRequest(context=self.CONTEXT, index=index)
    ...
    def put_index_across_all_labels(self):
        ...
        self.create_put_index_across_labels_request()

        if self.element_to_index == "VertexLabel":
            print("Request")
            print(self.REQUEST)
            return self.SERVICE.EnsureCompositeIndexForVertex(self.REQUEST)
        elif self.element_to_index == "EdgeLabel":
            return self.SERVICE.EnsureCompositeIndexForEdge(self.REQUEST)
        else:
            raise ValueError(f"Invalid element_to_index parameter. "
    ...
    def enable_index_by_name(self):
        self.__are_required_parameters_set__()
        self.create_enable_index_by_name_request()
        
        if self.element_to_index == "VertexLabel":
            return self.SERVICE.EnableVertexCompositeIndex(self.REQUEST)
        elif self.element_to_index == "EdgeLabel":
            return self.SERVICE.EnableEdgeCompositeIndex(self.REQUEST)
        else:
            raise ValueError(f"Invalid element_to_index parameter. "
                             f"Expecting VertexLabel/EdgeLabel for {self.element_to_index}")

    def put_index_by_label(self):
        print("I'm putting index by label")
        self.__are_required_parameters_set__()

        self.create_put_index_by_label_request()
    ...
    def get_indices_by_label(self):
        ...
        self.create_get_indices_by_label_request()
